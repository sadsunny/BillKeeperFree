...
public class BillNotificationService extends Service {
      ...
      int  uniqueCode = 0 ;
		...
		public void onStart(Intent intent, int startId) { //��ʼ��ѯ��ݿ⣬����������alarm
	        ...
	        thr.start();
//	        Toast.makeText(this, "service��ʼִ��", Toast.LENGTH_SHORT).show();
		}
...
//	        Toast.makeText(this, "service�Ѿ���kill", Toast.LENGTH_SHORT).show();
	    ...
	    Runnable mTask = new Runnable() {
	        public void run() {
	        	...
	        	getData(getNowMillis(),getNowMillis()+days30);
...
//	     	     	  Log.v("position", "服务bk_billisReminder"+bk_billisReminder);
//	     	     	  Log.v("position", "服务payState"+payState);
	     	          if ( bk_billisReminder == 1 ) {
	     	        	  
	     	        	long reminderTime = reminderDate(bk_billDuedate,bk_billReminderDate,bk_billReminderTime);
//	     	        	 Log.v("mdb", "reminderTime��С��"+reminderTime);
//	     	        	 Log.v("mdb", "getNowMillis()��С��"+System.currentTimeMillis());
//	     	        	 Log.v("mdb", "��������"+(reminderTime-System.currentTimeMillis()/1000));
	     	        	 Log.v("mdb", "reminderTime�ľ���ʱ�䣺"+getMilltoDate(reminderTime));
//	     	        	 Log.v("mdb", "���ڵľ���ʱ�䣺"+getMilltoDate(System.currentTimeMillis()));
	     	        	
	     	        	if (reminderTime > System.currentTimeMillis()) {
	     	        		
	     	        		Intent intent = new Intent(BillNotificationService.this,AlamrReceiver.class); 
	     	        		intent.putExtra("dataMap",(Serializable)iMap);
	     	        		
//		     	        	Bundle bundle = new Bundle();  
//		     	        	bundle.putString("bk_accountName", bk_accountName);  
//		     	        	bundle.putInt("bk_billAmountUnknown", bk_billAmountUnknown);
//		     	        	bundle.putDouble("billamount", billamount);
//		     	        	bundle.putLong("bk_billDuedate", bk_billDuedate);
//		     	        	intent.putExtras(bundle); 
		     	        	
		     	        	am = (AlarmManager) getSystemService(ALARM_SERVICE); 
		     	        	int uniqueRequestCode = (int)System.currentTimeMillis();  
		     	        	pi = PendingIntent.getBroadcast(BillNotificationService.this,  uniqueCode,  intent, PendingIntent.FLAG_CANCEL_CURRENT);  
		     	        	intentArray.add(pi);
...
//	        Toast.makeText(this, "���ѵ�id��"+a, Toast.LENGTH_SHORT).show();
	    ...
	    public List<Map<String, Object>> getData(long firstDayOfMonth, long lastDayOfMonth) //ch
	    {
		  ...
		  String sql = "select BK_Bill.* ,BK_Account.bk_accountName ,BK_Account.accounthasCategory  ,BK_Category.bk_categoryIconName ,BK_Category.bk_categoryName from BK_Bill,BK_Account,BK_Category where BK_Bill.bk_billisReminder = 1 and BK_Bill.billhasAccount = BK_Account._id and BK_Category._id = BK_Account.accounthasCategory and BK_Bill.bk_billisRepeat != 1 and BK_Bill.bk_billDuedate > "+firstDayOfMonth+" and BK_Bill.bk_billDuedate <= "+lastDayOfMonth;
		  ...
		  Cursor cursorEA = db.rawQuery(sql, null);
	   	 ...
	   	 cursorEA.close();
	   	 db.close();
	   	 mDataList.addAll(RecurringEventService.recurringData(this, firstDayOfMonth, lastDayOfMonth) ); //���ѭ��ʱ���¼�
